<template>
  <v-card flat>
    ...
    <v-card-text>
      ...
      <p class="text-h5 font-weight-bold text-center">Service Reports</p>
      ...
      <div>
        <v-list shaped>
          <v-list-item>
            ...
            <v-list-item-avatar width="15%"> Item </v-list-item-avatar>
            <v-list-item-avatar width="5%"> Quantity </v-list-item-avatar>
          ...
          <v-divider></v-divider>
            <div v-for="(item, index) in $store.state.Reports.service_reports.services" :key="index">
              <v-list-item>
    ...
    <v-card-text>
      <div id="excel" class="d-none">
        ...
        <table
          v-for="(item, index) in $store.state.Reports.service_reports.services"
          :key="index"
        >
          <tr>
            <td>{{ item.title }}</td>
...
export default {
  ...
  methods: {
    ...
    excel() {
      this.$excel("excel", `Service Reports ${this.date_title}`);
    },
  },
  ...
  computed: {
    ...
    total_price() {
      var services = this.$store.state.Reports.service_reports.services.reduce(
        (total, item) => {
