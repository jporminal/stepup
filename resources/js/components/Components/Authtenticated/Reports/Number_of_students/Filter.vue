<template>
  <v-card class="d-print-none">
    <v-row>
      <!-- <v-col cols="12" md="5">
        <v-date-picker
          v-model="$store.state.Reports.number_of_students_filter.dates"
          range
          full-width
        ></v-date-picker>
      </v-col> -->
        ...
        <v-card flat>
            ...
            <v-row justify="center">
              <v-col cols="12">
                <div class="text-h6 text-center">
                  Filtered Date: {{ new Date().toISOString().substr(0, 10) }}
                </div>
              </v-col>
              <v-col cols="12" md="6">
                <v-autocomplete
                  ...
                  item-value="drid"
                  v-model="$store.state.Reports.number_of_students_filter.term"
                  label="Select Term"
                />
              </v-col>
              <v-col cols="12" md="6">
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              :disabled="
                $store.state.Reports.number_of_students_filter.term > 0
                  ? false
                  : true
              "
              @click="filter"
...
export default {
  ...
  methods: {
    filter() {
      this.$store.commit(
        "NUMBER_OF_STUDENT_FILTER",
        this.$store.state.Reports.number_of_students_filter
      );
