<template>
  <v-dialog
    v-model="$store.state.Foremarkes.open"
    width="1000"
    @click:outside="close"
    persistent
  >
    <v-card>
      <v-container>
        <v-row align="center" justify="center">
          <v-col cols="12" md="8">
            <v-img
              :src="`/img/products/${$store.state.Foremarkes.product.image.image_name}`"
            >
            </v-img>
            <p class="title text-center mt-2">
              {{ $store.state.Foremarkes.product.ProductName }}
            </p>
            <v-select
              :items="$store.state.Foremarkes.product.option_name"
              item-text="item"
              label="Select Option"
              return-object
              solo
          ...
          <v-col cols="10">
            ...
            <div v-html="$store.state.Foremarkes.product.classnotes"></div>
          </v-col>
        </v-row>
      </v-container>
      <v-card-actions>
  ...
  methods: {
    ...
    add_cart() {
      ...
      var exist_item = this.$store.state.Carts.carts.find(
        (product) =>
          product.product_id ==
            this.$store.state.Foremarkes.product.ProductID &&
      ...
      var snackbar = {
        ...
        left: false,
        right: false,
        text: `${this.$store.state.Foremarkes.product.classname} (${this.option.item}) successfully added to your cart!`,
      };
