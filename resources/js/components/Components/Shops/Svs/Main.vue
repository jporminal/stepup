<template>
  <v-card flat :loading="$store.state.Svs.loading">
...
export default {
  ...
  methods: {
    channel(e) {
      var filter = {
        filter: this.$store.state.Svs.product_filter.filter,
        page: e,
        per_page: 12,
      };
    ...
    add_cart(e) {
      this.$store.dispatch("SVS_VIEW_ITEM", e);
    },
  },

  computed: {
    products() {
      return this.$store.state.Svs.products;
    },
  },
};
</script>
