<template>
  <v-dialog
    v-model="$store.state.Motor_city.open"
    width="1000"
    @click:outside="close"
    persistent
  >
    <v-card>
      <v-container>
        <v-row align="center" justify="center">
          <v-col cols="12" md="8">
            <v-img
              :src="$store.state.Motor_city.product.image !== null ? `/img/products/${$store.state.Motor_city.product.image.image_name}` : ''"
            >
            </v-img>
            <p class="title text-center mt-2">
              {{ $store.state.Motor_city.product.ProductName }}
            </p>
            <v-select
              :items="$store.state.Motor_city.product.option_name"
              item-text="item"
              label="Select Option"
              return-object
              solo
            ...
            <template v-if="Object.keys(option).length > 4">
              <p v-if="!$store.state.Date_ranges.store_setup.motorcity_lesson_ebd.value || in_array(option.product_id, productIdExcludedInEBD)">Price: {{ option.price | currency }}</p>
              <p v-if="$store.state.Date_ranges.store_setup.motorcity_lesson_ebd.value && !in_array(option.product_id, productIdExcludedInEBD)">Price: {{ ebd(option.price) | currency }} <sup style="color: red" v-if="option.price > 0"> <del> {{ option.price | currency }} </del> 10% off EBD</sup></p>
          ...
          <v-col cols="10">
            ...
            <div v-html="$store.state.Motor_city.product.classnotes"></div>
          </v-col>
        </v-row>
      </v-container>
      <v-card-actions>
...
export default {
  ...
  data() {
    return {
      option: {
        ...
        productIdExcludedInEBD: [5228, 5296, 5227,5239,5230, 5242, 5229, 5241, 5085,5061, 5086, 5062,5087]
    };
  },
  ...
  methods: {
    ...
    add_cart() {
      ...
      if(this.$store.state.Date_ranges.store_setup.motorcity_lesson_ebd.value && !this.in_array(this.option.product_id, this.productIdExcludedInEBD)) {
      ...
      var exist_item = this.$store.state.Carts.carts.find(
        (product) =>
          product.product_id ==
            this.$store.state.Motor_city.product.ProductID &&
      ...
      var snackbar = {
        ...
        left: false,
        right: false,
        text: `${this.$store.state.Motor_city.product.classname} (${this.option.item}) successfully added to your cart!`,
      };
