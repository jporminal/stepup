<template>
  <v-dialog
    v-model="$store.state.Arcadia.open"
    width="1000"
    @click:outside="close"
    persistent
  >
    <v-card>
      <v-container>
        <v-row align="center" justify="center">
          <v-col cols="12" md="8">
            <v-img
              :src="`/img/products/${$store.state.Arcadia.product.image.image_name}`"
            >
            </v-img>
            <p class="title text-center mt-2">
              {{ $store.state.Arcadia.product.ProductName }}
            </p>
            <v-select
              :items="$store.state.Arcadia.product.option_name"
              item-text="item"
              label="Select Option"
              return-object
              solo
          ...
          <v-col cols="10">
            ...
            <div v-html="$store.state.Arcadia.product.classnotes"></div>
          </v-col>
        </v-row>
      </v-container>
      <v-card-actions>
  ...
  methods: {
    ...
    add_cart() {
      ...
      var exist_item = this.$store.state.Carts.carts.find(
        (product) =>
          product.product_id ==
            this.$store.state.Arcadia.product.ProductID &&
      ...
      var snackbar = {
        ...
        left: false,
        right: false,
        text: `${this.$store.state.Arcadia.product.classname} (${this.option.item}) successfully added to your cart!`,
      };
